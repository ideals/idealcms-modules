...
class ModelAbstract extends \Ideal\Core\Site\Model
{
    ...
    public function getList($page, $onPage)
    {
        ...
        $_sql = "SELECT * FROM {$this->_table} WHERE is_active=1 AND structure_path='{$structurePath}'"
              . ' AND date_create < ' . time() . " ORDER BY date_create DESC LIMIT {$start}, {$onPage}";
    ...
    public function getListCount()
    {
        ...
        $_sql = "SELECT COUNT(ID) FROM {$this->_table} WHERE is_active=1 AND structure_path='{$this->structurePath}'";
    ...
    public function detectPageByUrl($url, $path)
    {
        ...
        $_sql = "SELECT * FROM {$this->_table} WHERE is_active=1 AND url='{$url}' ORDER BY date_create DESC";
