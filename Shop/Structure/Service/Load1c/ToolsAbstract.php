...
class ToolsAbstract
{
    ...
    protected string $tableCat = 'catalogplus_structure_category';

    /** @var string Таблица товаров */
    protected string $tableGood = 'catalogplus_structure_good';

    /** @var string Таблица c информацией о товарном предложении */
    protected string $tableOffer = 'catalogplus_structure_offer';

    /** @var string Таблица для связи категории и товара */
    protected string $tableMediumGood = 'catalogplus_medium_categorylist';
    ...
    public function showProperties(string $importFile, string $offersFile, $priceId): void
    {
        $base = new Model(DOCUMENT_ROOT . $importFile, DOCUMENT_ROOT . $offersFile, $priceId);
    ...
    public function showCategories(string $importFile, string $offersFile, $priceId): void
    {
        $base = new Model(DOCUMENT_ROOT . $importFile, DOCUMENT_ROOT . $offersFile, $priceId);
     ...
     * @param bool $conect1c
     */
    public function loadBase(string $importFile, string $offersFile, $priceId, $loadImg = false, $conect1c = false): void
    {
        $db = Db::getInstance();
        ...
        $sql = sprintf("UPDATE %s SET num = 0, count_sale = 0, is_not_menu = 0 WHERE id_1c != 'not-1c'", $this->tableCat);
        $db->query($sql);
        ...
        $sql = sprintf("SELECT ID, name, id_1c, is_active FROM %s WHERE prev_structure='%s'", $this->tableGood, $this->prevGood);
        $goods = $db->select($sql);
        ...
        $db->query(sprintf('UPDATE %s SET sell = NULL', $this->tableGood));
        ...
        $sql = sprintf("UPDATE %s SET is_not_menu = 1 WHERE id_1c != 'not-1c' AND num = 0 AND count_sale = 0", $this->tableCat);
        $db->query($sql);
        ...
        if (!$conect1c) {
            ...
            $mail->sent('top@neox.ru', 'top@neox.ru, morozov@neox.ru, help1@neox.ru, seo3@neox.ru');
            echo $text;
        }
    }

    /**
     * @param array<string, mixed> $changedGroups
     */
    protected function updateCategories(Db $db, $table, string $txt, array $changedGroups): string
    {
        foreach ($changedGroups['update'] as $v) {
            ...
            $db->update($table)->set($update)->where('ID=:id', ['id' => $v['ID']])->exec();
        }
        ...
        foreach ($changedGroups['delete'] as $v) {
            $this->listCat[$v['id_1c']] = 0;
            $par = ['is_active' => 1, 'is_not_menu' => 1];
            $db->update($table)->set($par)->where('ID=:id', ['id' => $v['ID']])->exec();
    ...
    /**
     * @param array<string, mixed> $changedGoods
     */
    protected function updateGoods(Db $db, $table, $txt, array &$changedGoods, $loadImg = false, $conect1c = false)
    {
        ...
        foreach ($changedGoods['update'] as $v) {
            ...
            unset($v['idGroup']);
            unset($properties);
            $db->update($table)->set($v)->where('ID=:id', ['id' => $v['ID']])->exec();
        }
     ...
     * @param array<string, mixed> $changedGoods
     */
    protected function updateOffers(Db $db, $txt, array &$changedGoods)
    {
        $fields = [
            'ID' => ['sql' => 'int(8) unsigned not null auto_increment primary key', 'label' => ''],
            'prev_structure' => ['sql' => 'varchar(15)', 'label' => ''],
            'offer_id' => ['sql' => 'varchar(37)', 'label' => ''],
            'good_id' => ['sql' => 'varchar(37)', 'label' => ''],
            'price' => ['sql' => 'int(11)', 'label' => ''],
            'name' => ['sql' => 'varchar(255)', 'label' => ''],
            'count' => ['sql' => 'int(11)', 'label' => ''],
        ];
        $_sql = 'DROP TABLE IF EXISTS ' . $this->tableOffer;
        $db->query($_sql);
        $db->create($this->tableOffer, $fields);
        $db->query(sprintf('ALTER TABLE %s ADD INDEX ( good_id )', $this->tableOffer));
     ...
     * @param array<string, mixed> $good Bнформация о товаре
     * @return string Url товара
     */
    protected function getUrl(array $good): string
    {
     ...
     */
    protected function loadImg(array &$v)
    {
        /*
    ...
    {
        $fields = [
            'category_id' => ['sql' => 'int(11)', 'label' => ''],
            'good_id' => ['sql' => 'int(11)', 'label' => ''],
        ];
        ...
        if ($status == 'full') {
            ...
            $db->query(sprintf('ALTER TABLE %s ADD INDEX ( category_id )', $this->tableMediumGood));
            $db->query(sprintf('ALTER TABLE %s ADD INDEX ( good_id )', $this->tableMediumGood));
            foreach ($goodsToGroups as $groupId => $goodIds) {
                foreach ($goodIds as $goodId) {
                    ...
                    $_sql = sprintf("SELECT ID FROM %s AS t1 WHERE t1.id_1c='%s' LIMIT 1", $this->tableCat, $groupId);
                    $id = $db->select($_sql);
                    $id = $id[0]['ID'];
                    $_sql = sprintf("SELECT ID FROM %s AS t1 WHERE t1.id_1c='%s' LIMIT 1", $this->tableGood, $goodId);
                    $id2 = $db->select($_sql);
                    $id2 = $id2[0]['ID'];
                    $row = [
                        'category_id' => $id,
            ...
            foreach ($goods as $goodId => $groupsId) {
                $_sql = sprintf("DELETE FROM %s WHERE good_id = '%s'", $this->tableMediumGood, $goodId);
                $db->query($_sql);
     ...
     */
    private function updateCountGoodsCat(Db $db, string $table, array $sale, $groups, array $countCat): int
    {
        $arr = [];
        $count = 0;
        ksort($groups);
        foreach ($groups as $key => $val) {
            preg_match_all('/(\d{0,3})/', $key, $tmp);
            $tmp = $tmp[0];
            ...
            $obiwan['count_good'] = $countCat[$val['id_1c']] ?? 0;
     ...
     */
    private function recurUpdateSale(Db $db, $table, array &$elem): array
    {
        $countS = 0;
        $countG = 0;
        if (isset($elem['sub'])) {
        ...
        $tmp = $elem['id_1c'];
        $_sql = sprintf('UPDATE %s SET count_sale=%s, num=%s', $table, $countS, $countG);
        $_sql .= sprintf(" WHERE id_1c='%s'", $tmp);
        $db->query($_sql);
        return ['s' => $countS, 'g' => $countG];
    }
}
