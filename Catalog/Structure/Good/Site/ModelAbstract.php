...
class ModelAbstract extends \Ideal\Core\Site\Model
{
    ...
    public function detectPageByUrl($path, $url)
    {
        ...
        $this->categoryModel = new \Catalog\Structure\Category\Site\Model($this->structurePath);
    ...
    public function detectCurrentCategory()
    {
        if (!isset($this->categoryModel)) {
            ...
            $this->categoryModel = new \Catalog\Structure\Category\Site\Model('');
    ...
    /*public function getAllGoods(){
        ...
        $tableCat = $config->db['prefix'] . 'catalog_structure_category';

        $where = getWhere("cid LIKE '{$cid}%'");

        $_sql = "SELECT * FROM {$this->_table} WHERE ID IN (SELECT I) {$where}";
